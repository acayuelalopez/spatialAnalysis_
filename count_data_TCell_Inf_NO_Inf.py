...
def process_files(input_directory, tumor_border_directory, output_directory, data_file):
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".csv"):  # and "TCell" not in filename:
            ...
            # Filter rows based on status for tumor distances
            if status == 'Inf':
                df_filtered = df[df['Parent'] == 'Annotation (Infiltrated)']
            else:
                df_filtered = df[df['Parent'] == 'Annotation (No_Infiltrated)']

            # Count the total number of cells
            total_cells = len(df_filtered)

            # Find the maximum length of strings in the Classification column
            max_length = df_filtered['Classification'].str.len().max()

            # Filter the DataFrame to keep only rows with the longest strings in the Classification column
            df_filtered = df_filtered[df_filtered['Classification'].str.len() == max_length]

            # Now you can proceed with counting the number of cells for each classification
            ...
            tumor_border_file = next((tb_file for tb_file in os.listdir(tumor_border_directory) if
                                      tb_file.endswith(".csv") and patient_id in tb_file and condition in tb_file),
                                     None)
            ...
            cell_coords = df_filtered[['Centroid X µm', 'Centroid Y µm']].values
            ...
            df_filtered['Min Distance to Tumor'] = distances
            # Filtrar distancias menores o iguales a 100 solo para el cálculo de distancias
            df_filtered_tumor = df_filtered[df_filtered['Min Distance to Tumor'] <= 100]

            # Calculate average, min, and max distances to tumor border for each classification
            avg_distance_to_tumor = df_filtered_tumor.groupby('Classification')['Min Distance to Tumor'].mean()
            min_distance_to_tumor = df_filtered_tumor.groupby('Classification')['Min Distance to Tumor'].min()
            max_distance_to_tumor = df_filtered_tumor.groupby('Classification')['Min Distance to Tumor'].max()
            ...
            max_elements_count = df_filtered['Classification'].dropna().apply(lambda x: len(x.split(':'))).max()

            # Remove values in Classification column that do not contain the maximum number of elements separated by ':'
            df_filtered.loc[
                df_filtered['Classification'].apply(lambda x: len(x.split(':')) != max_elements_count if isinstance(x,
    ...
    for tcell_filename in os.listdir(input_directory):
        if "TCell" in tcell_filename and tcell_filename.endswith(".csv"):
            ...
            if 'Classification' not in df_tcell.columns or "CD3+: CD16-: CD163-: HLA-DR-" not in df_tcell[
            ...
            tcell_cells = df_tcell[df_tcell['Classification'] == "CD3+: CD16-: CD163-: HLA-DR-"]
            ...
            for filename in os.listdir(input_directory):
                if filename.endswith(".csv"):
                    ...
                    if condition_pop == condition_tcell and patient_id_pop == patient_id_tcell:
                        print(patient_id_pop)
                        print(patient_id_tcell)
                        pop_file_path = os.path.join(input_directory, filename)
                        df_pop = pd.read_csv(pop_file_path, sep='\t')
                        # Filter rows based on status for tumor distances
                        if status_pop == 'Inf':
                            df_pop_filtered = df_pop[df_pop['Parent'] == 'Annotation (Infiltrated)']
                        else:
                            df_pop_filtered = df_pop[df_pop['Parent'] == 'Annotation (No_Infiltrated)']
                        cell_coords_pop = df_pop_filtered[['Centroid X µm', 'Centroid Y µm']].values
                        ...
                        df_pop_filtered['Min Distance to TCell'] = distances_tcell
                        # Filter distances less than or equal to 100 for distance calculations
                        df_pop_filtered_tcell = df_pop_filtered[df_pop_filtered['Min Distance to TCell'] <= 100]
                        # Find the maximum length of strings in the Classification column
                        max_length = df_pop_filtered_tcell['Classification'].str.len().max()
                        # Filter the DataFrame to keep only rows with the longest strings in the Classification column
                        df_pop_filtered_tcell = df_pop_filtered_tcell[
                            df_pop_filtered_tcell['Classification'].str.len() == max_length]
                        # Calculate average, min, and max distances to TCell cells for each classification
                        avg_distance_to_tcell = df_pop_filtered_tcell.groupby('Classification')[
                            'Min Distance to TCell'].mean()
                        min_distance_to_tcell = df_pop_filtered_tcell.groupby('Classification')[
                            'Min Distance to TCell'].min()
                        max_distance_to_tcell = df_pop_filtered_tcell.groupby('Classification')[
                            'Min Distance to TCell'].max()
                        ...
                        df_pop_filtered_tcell.to_csv(output_csv_file_tcell, index=False)
                        ...
                        if (population_pop, patient_id_pop) in quantification_results:
                            for result in quantification_results[(population_pop, patient_id_pop)]:
                                if result['Condition'] == condition_pop and result['Status'] == status_pop and result[
                            ...
                            print(
                                f"Updated quantification results for Population: {population_pop}, PatientID: {patient_id_pop}, Classification: {result['Classification']}")
                        else:
...
# Example usage
input_directory = "/mnt/imgserver/CONFOCAL/IA/Projects/2024/2024_01_29_egonzalez/data/eva-gonzalez/spacio1/new_analysis_def"
tumor_border_directory = "/mnt/imgserver/CONFOCAL/IA/Projects/2024/2024_01_29_egonzalez/data/eva-gonzalez/spacio1/output/tumor_masks"
output_directory = "/home/acayuela/Desktop/output_test"
# output_directory = "/mnt/imgserver/CONFOCAL/IA/Projects/2025/2025_2_28_egonzalez/test/output_def"
