...
def generate_violin_box_plots(input_directory, output_directory):
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".csv") and ("_TCell_distances.csv" in filename or "_tumor_distances.csv" in filename):
            ...
            # Filter rows where 'Min Distance to Tumor' is less than or equal to 100
            if "_tumor_distances.csv" in filename:
                df = df[df['Min Distance to Tumor'] <= 100]
        ...
        for population in populations:
            ...
            if not population_df.empty:
                ...
                for status in ['Inf', 'No_Inf']:
                    ...
                    tumor_border_file_pre = next((tb_file for tb_file in os.listdir(tumor_border_directory) if
                                                  tb_file.endswith(
                                                      ".csv") and patient_id in tb_file and 'PRE' in tb_file), None)
                    if tumor_border_file_pre:
                        tumor_border_path_pre = os.path.join(tumor_border_directory, tumor_border_file_pre)
                        tumor_border_df_pre = pd.read_csv(tumor_border_path_pre)
                        tumor_coords_pre = tumor_border_df_pre[['Centroid X (µm)', 'Centroid Y (µm)']].values
                        plt.scatter(tumor_coords_pre[:, 0], tumor_coords_pre[:, 1], c='gray', s=5, label='Tumor Border',
                                    alpha=0.2)
                    if distance_column_tumor in pre_df.columns:
                        ...
                        plt.title(
                            f'Scatter Plot of Cell Positions Colored by {distance_column_tumor} for Patient {patient_id} (PRE) - Status {status}')
                        plt.legend(title='Condition')

                    # Subfigure for POST Tumor
                    plt.subplot(1, 2, 2)
                    post_df = status_df[status_df['Condition'] == 'POST']
                    tumor_border_file_post = next((tb_file for tb_file in os.listdir(tumor_border_directory) if
                                                   tb_file.endswith(
                                                       ".csv") and patient_id in tb_file and 'POST' in tb_file), None)
                    if tumor_border_file_post:
                        tumor_border_path_post = os.path.join(tumor_border_directory, tumor_border_file_post)
                        tumor_border_df_post = pd.read_csv(tumor_border_path_post)
                        tumor_coords_post = tumor_border_df_post[['Centroid X (µm)', 'Centroid Y (µm)']].values
                        plt.scatter(tumor_coords_post[:, 0], tumor_coords_post[:, 1], c='gray', s=5,
                                    label='Tumor Border', alpha=0.2)
                    if distance_column_tumor in post_df.columns:
                        ...
                        plt.title(
                            f'Scatter Plot of Cell Positions Colored by {distance_column_tumor} for Patient {patient_id} (POST) - Status {status}')
                        plt.legend(title='Condition')
                    ...
                    if distance_column_tcell in pre_df.columns:
                        ...
                        plt.title(
                            f'Scatter Plot of Cell Positions Colored by {distance_column_tcell} for Patient {patient_id} (PRE) - Status {status}')
                        plt.legend(title='Condition')
                    ...
                    if distance_column_tcell in post_df.columns:
                        ...
                        plt.title(
                            f'Scatter Plot of Cell Positions Colored by {distance_column_tcell} for Patient {patient_id} (POST) - Status {status}')
                        plt.legend(title='Condition')
        ...
        for population in populations:
            ...
            for status in ['Inf', 'No_Inf']:
                ...
                if 'Min Distance to Tumor' not in status_df.columns or status_df['Min Distance to Tumor'].isna().all():
                    print(f"No valid 'Min Distance to Tumor' data for Patient {patient_id} - Population {population} - Status {status}")
                    continue
                plt.figure(figsize=(12, 6))
                sns.histplot(data=status_df, x='Min Distance to Tumor', hue='Condition', element='step', stat='percent', common_norm=False)
                plt.xlabel('Min Distance to Tumor')
                plt.ylabel('Density')
                plt.title(f'Histogram of Min Distance to Tumor for Patient {patient_id} - Population {population} - Status {status}')
                plt.legend(title='Condition', labels=['PRE', 'POST'])
                ...
                if 'Min Distance to TCell' not in status_df.columns or status_df['Min Distance to TCell'].isna().all():
                    print(f"No valid 'Min Distance to TCell' data for Patient {patient_id} - Population {population} - Status {status}")
                    continue
                plt.figure(figsize=(12, 6))
                sns.histplot(data=status_df, x='Min Distance to TCell', hue='Condition', element='step', stat='percent', common_norm=False)
                plt.xlabel('Min Distance to TCell')
                plt.ylabel('Density')
                plt.title(f'Histogram of Min Distance to TCell for Patient {patient_id} - Population {population} - Status {status}')
                plt.legend(title='Condition', labels=['PRE', 'POST'])
                ...
                for status in ['Inf', 'No_Inf']:
                    ...
                    if not pre_df.empty:
                        ...
                        if len(points_pre) >= 4:
                            ...
                            plt.scatter(tumor_coords_pre[:, 0], tumor_coords_pre[:, 1], c='gray', s=10, label='Tumor Border', alpha=0.5)
                    ...
                    if not post_df.empty:
                        ...
                        if len(points_post) >= 4:
                            ...
                            plt.scatter(tumor_coords_post[:, 0], tumor_coords_post[:, 1], c='gray', s=10, label='Tumor Border', alpha=0.5)
                            scatter_tumor_post = plt.scatter(post_df['Centroid X µm'],
                                                         post_df['Centroid Y µm'],
                                                         c=post_df[distance_column_tumor],
                                                         cmap='viridis', s=5)
...
# Example usage
input_directory = "/home/acayuela/Desktop/output_test_no_ck"
#input_directory = "/mnt/imgserver/CONFOCAL/IA/Projects/2025/2025_2_28_egonzalez/test/output_def"
output_directory = "/mnt/imgserver/CONFOCAL/IA/Projects/2025/2025_2_28_egonzalez/def/figs_No_CK+"
